...
export default function Home() {
  ...
  function openNewEvent() {
    ...
    setEventDraft(emptyEvent(state.preferences.defaultCollector));
    setEventFiles([]);
    setEventModal(true);
  }
  ...
  async function runCompleteExport() {
    ...
    try {
      ...
      setNotice(
        "Complete export downloaded: Excel previews plus full-resolution photos.",
      );
    } catch {
  ...
  async function runExcelExport() {
    setExportBusy(true);
    try {
      await downloadXlsxWithPhotos(
        exportData.events,
        exportData.specimens,
        `EntoLabel_import_${todayParts().date}.xlsx`,
      );
      setNotice(
        "Excel downloaded with a clean import sheet and Visual specimen catalog.",
      );
    } catch {
      setNotice("The Excel file could not be created. Please try again.");
    } finally {
      setExportBusy(false);
    }
  }
      ...
      {!events.length ? (
        <div className="empty-state">
          <img
            ...
            loading="eager"
          />
          <p className="eyebrow">Ready for the field</p>
          <h2>Your first event starts with one tap.</h2>
          <p>
        ...
        <div className="event-grid">
          {events.map((event, index) => {
            ...
            const individuals = records.reduce(
              (sum, record) => sum + record.quantity,
              0,
            );
            return (
              <article
                className={`event-card ${index ? "is-compact" : ""}`}
                key={event.id}
              >
                ...
                <div className="event-details">
                  <p className="sample-label">
                    {event.isExample ? "Example event" : "Collecting event"}
                  </p>
                  <h2>{event.id}</h2>
                  <p className="event-location">
                    <MapPin aria-hidden="true" />
                    {event.locality} · {event.date} {event.time}
                  </p>
                  <p className="coordinates">{formatCoordinates(event)}</p>
                  <div className="badges">
                    ...
                    <span className="badge specimen-badge">
                      <Bug aria-hidden="true" />
  ...
  return (
    <section>
      <button className="text-button" onClick={onBack}>
        <ArrowLeft aria-hidden="true" /> All events
      </button>
    ...
    <section>
      ...
      <div className="export-grid">
        <ExportCard
          ...
          description="Best default. A clean import table, a visual specimen catalog with photos in each row, and a full photo index."
          button={busy ? "Preparing photos…" : "Download .xlsx"}
          recommended
